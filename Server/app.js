...
io.on('connection', function (socket) {	//'connection' (1) nay khac gi voi 'connection' (2)
    ...
    console.log('signup_member')
    //socket.broadcast.emit('request_car_code');
    querryObj_parking =  { 'id': message.id,
                            'name_parking_lot': message.name };
        MongoClient.connect(mongodbUrl, function (err, db) {
            assert.equal(null, err);
        ...
        MongoClient.connect(mongodbUrl, function (err, db) {
            ...
            db.collection("user").findOne(querryObj, function (err, result) {
                ...
                if ((result) && (result.parking_flag == 0)) {
                    ...
                    db.collection("user").find(querryObj2).toArray(function (err, result) {
                        if (err) throw err;
                       socket.broadcast.emit('res_b', result.length)
                        db.close();
                    });*/
                    } else {
                    console.log("2")
    ...
    socket.on('response_car_code', function (message) {
        ...
        MongoClient.connect(mongodbUrl, function (err, db) {
           /* var querryObj2 = {
                        'parking_flag': 1,
                        'name_parking_lot': querruObj_parking.name_parking_lot
                    };
                    db.collection("user").find(querryObj2).toArray(function (err, result) {
                        if (err) throw err;
                        socket.emit('res_b', result.length)
